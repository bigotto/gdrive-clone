...
describe('#UploadHandler test suites', () => {
  const ioObj = {
    to: (id) => ioObj,
    emit: (event, message) => {}
  }
  ...
  describe('#registerEvents', () => {
    test('should call onFile and onFinish functions on Busboy instance', () => {
      ...
      // busboyInstance.emit('finish', 'aaa')
  ...
  describe('#onFile', () => {
    test('given a stream file it should save it on disk', async () => {
      ...
      const expectedFilename = resolve(handler.donwloadsFolder, params.filename)
  ...
  describe('#handleFileBytes', () => {
    ...
    test('given messages timerDelay as 2secs it should emit only two messages during 2 seonds period', async () => {
      jest.spyOn(ioObj, ioObj.emit.name)
